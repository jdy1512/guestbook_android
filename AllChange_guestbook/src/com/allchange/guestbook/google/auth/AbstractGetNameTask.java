...
package com.allchange.guestbook.google.auth;
...
import com.allchange.guestbook.login.GoogleLogin;
...
public abstract class AbstractGetNameTask extends AsyncTask<Void, Void, Void> {
	...
	protected MainLoginActivity mActivity;
	...
	AbstractGetNameTask(MainLoginActivity activity, String email, String scope) {
	...
	protected void onError(String msg, Exception e) {
		...
		GoogleLogin.getInstance(mActivity).show(msg); // will be run in UI
														// thread
	}
	...
	private void fetchNameFromProfileServer() throws IOException, JSONException {
		...
		if (sc == 200) {
			...
			getFirstName(readResponse(is));
			// GoogleLogin.getInstance(mActivity).show("Hello " + name + "!");
			is.close();
			return;
		} else if (sc == 401) {
	 ...
	 *             in response
	 */
	private void getFirstName(String jsonResponse) throws JSONException {
		JSONObject profile = new JSONObject(jsonResponse);

		// if (mListener != null) {
		// try {
		// mListener.FBGoogleCallback("google", mToken, "", "", "", "",
		// profile.getString(NAME_KEY),
		// profile.getString("family_name"),
		// profile.getString("locale"),
		// profile.getString("gender"), profile.getString("id"));
		// } catch (JSONException e) {
		// e.printStackTrace();
		// }
		// }
		...
		PropertyManager.getInstance().setGoogleId(profile.getString("id"));
		AndroidQuery.getInstance().requestGoogleLogin(mActivity,
				Url.SERVER_GOOGLE_LOGIN, this.mEmail,
				profile.getString(NAME_KEY), profile.getString("family_name"),
				gender, profile.getString("picture"), profile.getString("id"));
